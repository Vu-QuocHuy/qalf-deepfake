...
OUTPUT_DIR="$STORAGE_ROOT/experiments/qalf_ffpp4_effb0_160_12f_attention_ema"

echo "Python: $PYTHON"
echo "Training output: $OUTPUT_DIR"
echo "Protocol: EfficientNet-B0 160px, 12 texture frames, temporal attention, EMA=0.999"
export CUBLAS_WORKSPACE_CONFIG=':4096:8'
echo "Deterministic cuBLAS workspace: $CUBLAS_WORKSPACE_CONFIG"

"$PYTHON" scripts/train.py \
    --config configs/ffpp_to_celebdf_b0_12f_attention_ema.json \
    ...
    --texture-temporal-pooling attention \
    --geometry-hidden 128 \
    --geometry-layers 3 \
    --embedding-dim 192 \
    --dropout 0.3 \
