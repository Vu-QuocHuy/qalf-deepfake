...
def crop_face_to_256(image_rgb: np.ndarray, mtcnn_detector) -> tuple[np.ndarray, np.ndarray]:
    """Crop face to 256x256 using MTCNN (matches official pipeline). Returns (crop, bbox)."""
    ...
    try:
        ...
        if boxes is not None and len(boxes) > 0 and boxes[0] is not None:
            ...
            if crop.shape[0] > 16 and crop.shape[1] > 16:
                return cv2.resize(crop, (256, 256), interpolation=cv2.INTER_AREA), box
    except Exception:
        pass
...
def main():
    ...
    mtcnn = MTCNN(image_size=256, margin=0, keep_all=False, post_process=False, device="cpu")
